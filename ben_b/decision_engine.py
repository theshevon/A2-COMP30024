...
class DecisionEngine():
    ...
    def evaluate_state(self, board, state, colour):
        ...
        player_exits        =state.exit_counts[colour]
        if n_nodes == 0:
        ...
        for p_node in player_nodes:
            p_moves = board.get_min_no_of_moves_to_exit(p_node, colour)
            #counts how many pieces are near the boards edge
            if p_moves <=2:
                pieces_near_exit+=1

            total_no_moves_to_exit += p_moves
        ...
        exit_score =0 
        if pieces_near_exit >= 4- player_exits :
            exit_score+= player_exits *10
        else:
            exit_score+= player_exits*1
